...
似然函数 $Pr (\mathbf x |\mathbf z, \phi)$ 也服从正态分布；其均值 $\text{f} [\mathbf z, \phi]$ 是潜变量的非线性函数，协方差 $σ^{2} \mathbf I$ 是球形的：
...
函数 $\text{f} [\mathbf z, \phi]$ 是由具有参数 $\phi$ 的深度网络来描述的。潜变量 $\mathbf z$ 的维度低于数据 $\mathbf x$ 。模型 $\text{f} [\mathbf z, \phi]$ 描述了数据的重要方面，而剩下未建模的方面则归因于噪声 $σ^{2} \mathbf I$ 。

通过对潜变量 $\mathbf z$ 的边缘化，可以找到数据概率 $Pr (\mathbf x|\phi)$ ：
...
我们可以将它视为具有不同均值的球形高斯分布的无限加权和（即无限混合），其中权重为 $Pr(\mathbf z)$ ，均值为网络输出 $\text{f}[\mathbf z, \phi]$ （图[17.2]()）。

![Alt text](../img/image-17.2.png)
> 图17.2 非线性潜变量模型。通过对潜在变量 $z$ 上的联合分布 $Pr(\mathbf x, z)$ （左图）进行边缘化，我们创建了一个复杂的二维密度 $Pr(\mathbf x)$ （右图）；为了创建 $Pr(\mathbf x)$ ，我们对维度 $z$ 上的三维体积进行积分。对于每个 $z$ ， $x$ 上的分布是一个球形高斯分布（显示了两个切片），其均值 $\text{f}[z, \phi]$ 是 $\mathbf z$ 的非线性函数，并且依赖于参数 $\phi$ 。分布 $Pr(\mathbf x)$ 是这些高斯分布的加权和。

### 17.2.1 生成 - Generation

使用祖先采样（图[17.3]()）可以生成新的示例 $\mathbf x^{∗}$ 。我们从先验分布 $Pr(\mathbf z)$ 中抽取 $z^{∗}$ ，并通过网络 $\text{f}[\mathbf z^{∗}, \phi]$ 计算似然 $Pr(\mathbf x|\mathbf z^{∗}, \phi)$ 的均值（方程[17.6]()），然后我们抽取 $\mathbf x^{∗}$ 。先验分布和似然都是正态分布，因此这一步非常简单。

![Alt text](../img/image-17.3.png)
> 图17.3 非线性潜变量模型的生成过程。
>
> * a) 我们从潜变量的先验概率 $Pr(z)$ 中抽取一个样本 $z^{∗}$ 。
> * b) 然后从 $Pr(\mathbf x|z^{∗}, \phi)$ 中抽取样本 $\mathbf x^{∗}$ 。这个分布是一个球形高斯分布，其均值是一个关于 $z^{∗}$ 的非线性函数 $\text{f}[•, \phi]$ ，方差是固定的。
> * c) 如果我们多次重复这个过程，最后我们可以得到密度 $Pr(\mathbf x|\phi)$ 的分布。

## 17.3 训练 - Training

为了训练模型，我们通过最大化对于模型参数的对数似然函数在训练数据集 $\{x_{i}\}^I_{i=1}$ 上进行训练。为了简化起见，我们可以假设似然函数中的方差项 $σ^{2}$ 是已知的，并集中于学习 $\phi$ ：

$$
\hat{\phi}\quad=\quad\underset{\phi}{\mathop{\text{argmax}}}\left[\sum_{i=1}^{I}\log\Bigl[Pr(\mathbf{x}_{i}|\phi)\Bigr]\right]
\qquad\text{(17.8)}
$$
...
为了取得进展，我们定义了一个对数似然的*下界*。对于给定的 $\phi$ 值，该函数始终小于或等于对数似然，并且还依赖于其他一些参数 $\theta$ 。最终，我们将构建一个网络来计算这个下界并进行优化。为了定义这个下界，我们需要使用*Jensen不等式*。

### 17.3.2 Jensen不等式 - Jensen's inequality

Jensen不等式表明，对于数据 $y$ 的期望的凹函数 $\text{g} [•]$ 大于或等于函数对数据的期望：
...
> 对数函数（黑色曲线）是一个凹函数；你可以在曲线上的任意两点之间画一条直线，这条直线总是在曲线下方。因此，对数函数上的任何凸组合（权重和为1的正权重加权和）必须位于曲线下方的灰色区域中。在这里，我们将这些点均等加权（即取均值），得到青色点。由于这个点位于曲线下方，所以 $\text{log}[\mathbb{E} [y]] > \mathbb{E}[\text{log} [y]]$ 。

![Alt text](../img/image-17.5.png)
> 图 17.5 Jensen不等式（连续情况）。对于一个凹函数，计算分布 $Pr (y)$ 的期望，并通过函数传递它的结果大于或等于通过函数转换变量 $y$ 然后计算新变量的期望。以对数函数为例，我们有 $\text{log}[\mathbb{E}[y]] ≥ \mathbb{E}[\text{log}[y]]$ 。图的左侧对应于不等式的左侧，图的右侧对应于右侧。可以这样理解，我们将橙色分布中定义在 $y \in [0, 1]$ 上的点看作是凸组合。根据图[17.4]()的逻辑，这些点必然位于曲线下方。另一种思考方式是，凹函数相对于低值将 $y$ 的高值进行压缩，因此当我们首先通过函数传递 $y$ 时，期望值会较低。
...
在实践中，分布 $q (\mathbf z)$ 是由参数 $\theta$ 控制的，因此 ELBO 可以被写成：
...
为了学习非线性潜变量模型，我们通过最大化这个量来确定 $\phi$ 和 $\theta$ 的取值。 VAE 是一种计算这个量的神经网络结构。

## 17.4 ELBO 概念 - ELBO properties

当我们第一次遇到ELBO时，它可能是一个有些神秘的概念，因此现在我们提供一些关于它性质的直觉理解。想象一下，原始数据的对数似然是参数 $\phi$ 的一个函数，我们想要找到它的最大值。对于任何固定的 $\theta$ ，ELBO仍然是参数的一个函数，但它必须位于原始似然函数的下方。当我们改变 $\theta$ 时，我们改变了这个函数，而且根据我们的选择，这个下界可能会离对数似然越来越近或者越来越远。当我们改变 $\phi$ 时，我们沿着下界函数移动（参见图[17.6]()）。

![Alt text](../img/image-17.6.png)
> 图 17.6 证据下界（ELBO）(ELBO)。
> 目标是最大化对参数 $\phi$ 的对数似然 $\text{log}[Pr(\mathbf{x}|\phi)]$ （黑色曲线）。ELBO是一个在对数似然下方的函数。它既与参数 $\phi$ 有关，也与第二个参数集 $\theta$ 有关。对于固定的 $\theta$ ，我们得到一个关于 $\phi$ 的函数（不同 $\theta$ 值的两个彩色曲线）。因此，我们可以通过改善ELBO来提高对数似然，要么是通过改变新的参数 $\theta$ （从橙色曲线到绿色曲线），要么是通过改变原始参数 $\phi$ （沿着当前的彩色曲线移动）。
...
$$
\begin{aligned}
\text{ELBO}[\theta,\phi]& =\quad\int q(\mathbf{z}|\boldsymbol{\theta})\log\left[{\frac{Pr(\mathbf{x},\mathbf{z}|\boldsymbol{\phi})}{q(\mathbf{z}|\boldsymbol{\theta})}}\right]d\mathbf{z} \\\\
...
在这里，第一个积分在第三行和第四行之间消失，因为 $\text{log}[Pr (\mathbf x|\phi) ]$ 不依赖于 $\mathbf z$ ，而概率分布 $q (\mathbf z|\theta)$ 的积分为1。在最后一行，我们使用了Kullback-Leibler (KL) 散度的定义。

这个方程表明ELBO是原始对数似然减去KL散度 $D_{KL}[q(\mathbf z|\theta)||Pr(\mathbf z|\mathbf x, \phi)]$ 。KL散度衡量分布之间的“距离”，只能取非负值。因此，ELBO是对 $\text{log}[Pr (\mathbf x|\phi) ]$ 的下界。当 $q (\mathbf z|\theta) = Pr (\mathbf z|\mathbf x, \phi)$ 时，KL距离为零，下界将会“严密”。这是关于潜在变量 $\mathbf z$ 的后验分布，给定观测数据 $\mathbf x$ ，它指示了哪些潜在变量的值可能对数据点负责（图[17.7]()）。

![Alt text](../img/image-17.7.png)
> 图17.7 潜在变量的后验分布。
a) 后验分布 $Pr(z|\mathbf x^{∗}, \phi)$ 是对潜在变量 $z$ 负责的可能值的分布，可以解释数据点 $\mathbf x^{∗}$ 。我们通过应用贝叶斯定理计算它 $Pr(z|\mathbf x^{∗}, \phi)∝ Pr(\mathbf x^{∗}|z, \phi)Pr(z)$ 。
b) 我们通过将 $\mathbf x^{∗}$ 与与每个 $\mathbf z$ 值相关的对称高斯分布相比较来计算右边的第一项（似然）。在这里，它更可能是由 $z_{1}$ 而不是 $z_{2}$ 生成的。第二项是潜变量的先验概率 $Pr (z)$ 。将这两个因素结合并进行标准化，使得分布总和为一，我们得到了后验分布 $Pr (\mathbf z|x^{∗}, \phi)$ 。
...
在第一行和第二行之间，联合分布 $Pr (\mathbf x, \mathbf z|\phi)$ 被分解为条件概率 $Pr (\mathbf x|\mathbf z, \phi) Pr (\mathbf z)$ ，并且在最后一行再次使用了KL散度的定义。

在这个公式中，第一项衡量潜变量和数据之间的平均一致性 $Pr (\mathbf x|\mathbf z, \phi)$ ，也被称为重构损失。第二项衡量辅助分布 $q (\mathbf z|\theta)$ 与先验分布的匹配程度。这个公式是变分自编码器中使用的公式。

## 17.5 变分逼近 - Variational approximation

在方程[17.17]()中我们看到，当 $q (\mathbf z|\theta)$ 等于后验分布 $Pr (\mathbf z|\mathbf x, \phi)$ 时，ELBO是最紧密的。从原则上讲，我们可以使用贝叶斯规则计算后验分布：

$$
Pr(\mathbf{z}|\mathbf{x},\phi)=\frac{Pr(\mathbf{x}|\mathbf{z},\phi)Pr(\mathbf{z})}{Pr(\mathbf{x}|\boldsymbol{\phi})},
\qquad\text{(17.19)}
$$

但在实践中，这是不可行的，因为我们无法在分母中评估数据的似然性（见第[17.3](#training-1)节）。

一种解决方法是进行变分近似：我们选择一个简单的参数形式 $q (\mathbf z|\theta)$ 来近似真实的后验分布。在这里，我们选择均值为 $µ$ ，协方差为对角矩阵 $Σ$ 的多变量正态分布。这种选择不一定能完全匹配后验分布，但对于某些 $µ$ 和 $Σ$ 的值会更好一些。在训练过程中，我们会找到与真实后验分布 $Pr (\mathbf z|\mathbf x)$ 最接近的正态分布（见图[17.8]()）。这相当于最小化方程[17.17]()中的KL散度，并将图[17.6]()中的彩色曲线向上移动。

![Alt text](../img/image-17.8.png)
> 图17.8展示了变分近似的过程。后验 $Pr (\mathbf z| \mathbf x^{∗}, \phi)$无法以闭合形式计算。为了近似后验分布，变分近似选择了一族分布 $q (\mathbf z|\mathbf x, \theta)$（这里选择高斯分布），并试图找到这族分布中与真实后验最接近的成员。
>
> * a) 有时候，近似分布（青色曲线）能够很好地拟合真实后验分布（橙色曲线）。
> * b) 然而，如果真实后验分布是多峰的（如图[17.7]()所示），高斯近似将会表现不佳。

由于最优的 $q (\mathbf z|\theta)$选择是后验分布 $Pr (\mathbf z|\mathbf x)$，而后验分布又依赖于数据样本 $\mathbf x$，因此变分近似应该采用相同的策略，因此我们选择：

$$
q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})=\mathrm{Norm}_{\mathbf{z}}\bigg[\mathbf{g}_{\mu}[\mathbf{x},\boldsymbol{\theta}],\mathbf{g}_{\Sigma}[\mathbf{x},\boldsymbol{\theta}]\bigg],
\qquad\text{(17.20)}
$$

其中 $\mathbf g [\mathbf x, \theta]$ 是一个带有参数 $\theta$ 的第二个神经网络，用于预测正态变分逼近的均值 $µ$ 和方差 $Σ$ 。
...
$$
\mathrm{ELBO}[\theta,\phi]=\int q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\log\bigl[Pr(\mathbf{x}|\mathbf{z},\boldsymbol{\phi})\bigr]d\mathbf{z}-\mathrm{D}_{KL}\biggl[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\biggl|\biggl|Pr(\mathbf{z})\biggr],
\qquad\text{(17.21)}
$$

其中，分布 $q (\mathbf z|\mathbf x, \theta)$ 是方程[17.20]()中的近似分布。

第一项仍然包含一个难以解决的积分，但由于它是关于 $q (\mathbf z|\mathbf x, \theta)$ 的期望，我们可以通过采样来近似它。对于任何函数 $a [•]$ ，我们有：

$$
\mathbb{E}_{\mathbf{z}}\big[\mathrm{a}[\mathbf{z}]\big]=\int\mathrm{a}[\mathbf{z}]q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})d\mathbf{z}\approx\frac{1}{N}\sum_{n=1}^{N}\mathrm{a}[\mathbf{z}_{n}^{\ast}],
\qquad\text{(17.22)}
$$

其中 $\mathbf z^{∗}_n$ 是从 $q (\mathbf z|\mathbf x, \theta)$ 中采样得到的第 $n$ 个样本。这被称为*蒙特卡洛估计*。对于一个非常粗略的估计，我们可以只使用从 $q (\mathbf z|\mathbf x, \theta)$ 中获得的单个样本 $\mathbf z^{∗}$ ：

$$
\mathrm{ELBO}[\theta,\phi]\quad\approx\quad\log\big[Pr(\mathbf{x}|\mathbf{z}^{\ast},\phi)\big]-\mathrm{D}_{KL}\Big[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\Big|\Big|Pr(\mathbf{z})\Big].
$$

第二项是变分分布 $q (\mathbf z|\mathbf x, \theta) = Norm_{z}[µ, Σ]$ 和先验分布 $Pr (\mathbf z) = Norm_{z}[0,\mathbf I]$ 之间的KL散度。两个正态分布之间的KL散度可以通过闭式计算得到。对于其中一个分布的参数为 $\mathbf µ, \mathbf Σ$ ，而另一个分布是标准正态分布的特殊情况，KL散度可以表示为：

$$
\text{D}_{KL}\Big[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\Big|\Big|Pr(\mathbf{z})\Big]={\frac{1}{2}}\bigg(\text{Tr}[\mathbf{\Sigma}]+\mu^{T}\mathbf{\mu}-D_{\mathbf{z}}-\log\Big[\det[\mathbf{\Sigma}]\Big]\bigg).
\qquad\text{(17.24)}
$$
...
* 使用网络 $\mathbf g [\mathbf x, \theta]$ 来计算变分后验分布 $q (\mathbf z|\mathbf \theta, \mathbf x)$ 的均值 $µ$ 和方差 $Σ$ ，对于给定的数据点 $\mathbf x$
* 从该分布中抽取一个样本 $\mathbf z^{∗}$
* 使用指定的方程式[17.23]()计算证据下界（ELBO）

图[17.9]()展示了相关的架构。现在应该明白为什么它被称为变分自编码器了。它之所以称为变分自编码器，是因为它对后验分布进行了高斯近似计算。它也被称为自编码器，是因为它从数据点 $\mathbf x$ 开始，计算出一个较低维度的潜变量 $\mathbf z$ ，然后尽可能地利用这个向量重新创造数据点 $\mathbf x$ 。在这个背景下，网络 $\mathbf g [\mathbf x, \theta]$ 将数据映射到潜变量被称为*编码器*，而网络 $\text{f} [\mathbf z, \phi]$ 将潜变量映射到数据被称为*解码器*。

![Alt text](../img/image-17.9.png)

> 图 17.9 变分自编码器。编码器 $\mathbf g [\mathbf x, \theta]$ 接受一个训练示例 $\mathbf x$ 并预测变分分布 $q (\mathbf z|\mathbf x, \theta)$ 的参数 $µ, Σ$ 。我们从该分布中进行采样，然后使用解码器 $\text{f} [\mathbf z, \phi]$ 预测数据 $\mathbf x$ 。损失函数是负 ELBO，取决于预测的准确性以及变分分布 $q (\mathbf z|\mathbf x, \theta)$ 与先验分布 $Pr (\mathbf z)$ 的相似程度（方程 [17.21]() ）。

 VAE 使用 $\phi$ 和 $\theta$ 计算ELBO作为界限。为了最大化这个界限，我们通过网络运行小批量样本，并使用优化算法（如SGD或Adam）更新这些参数。使用自动微分通常计算参数相对于ELBO的梯度。在这个过程中，我们在图17.10中同时在彩色曲线之间移动（改变 $\theta$ ）和沿着曲线移动（改变 $\phi$ ）。在这个过程中，参数 $\phi$ 被改变，使得数据在非线性潜在变量模型中具有更高的可能性。

![Alt text](../img/image-17.10.png)
> 图17.10 VAE 通过调整解码器的参数 $\phi$ 和编码器的参数 $\theta$ 来增加下界，从而更新每次迭代时决定下界的两个因素。

## 17.7 重新参数化技巧 - The reparameterization trick

还有一个更为复杂的问题；网络中涉及到一个采样步骤，对这个随机组件进行微分是困难的。然而，为了更新网络中在其之前的参数 $\theta$ ，对采样步骤进行微分是必要的。

幸运的是，我们有一个简单的解决方法；我们可以将随机部分转移到网络的一个分支中，从 $Norm_{ϵ}[0,\mathbf I]$ 中抽取一个样本 $ϵ^{∗}$ ，然后使用关系式：

$$
\mathbf{z}^{\ast}=\mu+\Sigma^{1/2}\epsilon^{\ast}
\qquad\text{(17.25)}
$$
...
> 图17.12 在CELEBA数据集上训练的标准 VAE 进行采样。每一列中，先绘制一个潜变量 $\mathbf z^{∗}$ ，然后通过模型预测均值 $\text{f} [\mathbf z^{∗}, \phi]$ ，最后添加独立的高斯噪声（参见图[17.3]()）。
>
...
鼓励解缠的方法通常通过在损失函数中添加正则化项来实现，这些正则化项基于两种情况：(i) 潜变量后验概率 $q(\mathbf{z}|\mathbf{x},\theta)$ ，或者 (ii) 聚合后验概率 $q (\mathbf z|\theta) = (1/I)\sum_{i}q(\mathbf z|\mathbf x_{i}, \theta)$ ：

$$
L_{\mathrm{new}}=-\mathrm{ELBO}[\theta,\phi]+\lambda_{1}\mathbb{E}_{Pr(\mathbf{x})}\Big[\mathrm{r}_{1}\Big[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\Big]\Big]+\lambda_{2}\mathrm{r}_{2}\Big[q(\mathbf{z}|\boldsymbol{\theta})\Big].
\qquad\text{(17.29)}
$$
...
$$
\mathrm{ELBO}[\theta,\phi]\quad\approx\quad\log\big[Pr(\mathbf{x}|\mathbf{z}^{\ast},\phi)\big]-\beta\cdot\mathrm{D}_{KL}\Big[q(\mathbf{z}|\mathbf{x},\boldsymbol{\theta})\Big|\Big|Pr(\mathbf{z})\Big],
\qquad\text{(17.30)}
$$
...
当分布 $q (\mathbf z|\theta)$ 与后验 $Pr (\mathbf z|\mathbf x, \phi)$ 匹配时，ELBO（证据下界（ELBO））是紧密的。这是*期望最大化*（EM）算法（[Dempster et al.，1977]()）的基础。在这里，我们交替进行以下两个步骤：（i）选择 $\theta$ 使得 $q (\mathbf z|\theta)$ 等于后验 $Pr (\mathbf z|\mathbf x, \phi)$ ，（ii）改变 $\phi$ 以最大化下界（图[17.15]()）。对于像高斯混合模型这样的模型，我们可以通过计算得到封闭形式的后验分布。不幸的是，对于非线性潜变量模型，情况并非如此，因此不能使用这种方法。

![Alt text](../img/image-17.15.png)
> 图 17.15 期望最大化（EM）算法。EM算法交替调整辅助参数 $\theta$ （在彩色曲线之间移动）和模型参数 $\phi$ （沿着彩色曲线移动），直到达到极大值。这些调整分别称为E步和M步。由于E步使用后验分布 $Pr (h|\mathbf x , \phi)$ 用于 $q (h|\mathbf{x} , \theta)$ ，因此界限很紧，每次E步之后，彩色曲线都会触碰到黑色似然曲线。
...
$$
g\big[\text{E}[y]\big]\leq\text{E}\big[g[y]\big].
\qquad\text{(17.31)}
$$
...
**问题 17.4**∗ 证明 ELBO，如方程 [17.18]() 所示，可以通过变分分布 $q (\mathbf z|\mathbf x)$ 和真实后验分布 $Pr (\mathbf z|\mathbf x, \phi)$ 之间的 KL 散度来推导出来：
