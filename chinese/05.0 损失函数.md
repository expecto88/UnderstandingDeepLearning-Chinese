...
$$
\hat{\phi} = \underset{\boldsymbol{\phi}}{\text{argmin}}\left[ L[\boldsymbol{\phi}] \right] = \underset{\boldsymbol{\phi}}{\text{argmin}}\left[ -\sum_{i=1}^I\text{log}\left[ Pr(\mathbf{y_{i}}|f[\mathbf{x_i},\boldsymbol{\phi}])\right] \right] \tag{5.6}
$$
...
$$
L[\phi] = \sum_{i=1}^I(y_{i} - f [x_{i}, \phi])^{2}.\tag{5.11}
$$
...
$$
\hat{\boldsymbol{\phi}},\hat{\boldsymbol{\sigma}}^2=\underset{\phi,\sigma^2}{\mathop{\text{argmin}}}\left[-\sum_{i=1}^I\log\left[\frac1{\sqrt{2\pi\sigma^2}}\exp\left[-\frac{(y_i-\mathrm{f}[\mathbf{x}_i,\boldsymbol{\phi}])^2}{2\sigma^2}\right]\right]\right].\tag{5.13}
$$
...
$$
µ = f_{1} [x, \phi]\\\\
σ^{2} = f_{2} [x, \phi]^2 \tag{5.14}
$$

这导致了损失函数：

$$
\hat{\phi}=\underset{\phi}{\mathop{\text{argmin}}}\left[-\sum_{i=1}^I\log\left[\frac1{\sqrt{2\pi\mathrm{f}_2[\mathbf{x}_i,\boldsymbol{\phi}]^2}}\right]-\frac{(y_i-\mathrm{f}_1[\mathbf{x}_i,\boldsymbol{\phi}])^2}{2\mathrm{f}_2[\mathbf{x}_i,\boldsymbol{\phi}]^2}\right].\tag{5.15}
$$
...
$$
\text{sig[z]} = \frac{1}{1 + \text{exp[-z]}}\tag{5.18}
$$
...
$$
Pr(y|x)  = (1 - \text{sig}[f [x, \phi]])^{1-y} \cdot \text{sig}[f [x, \phi]]^{y}\tag{5.19}
$$
...
$$
\text{softmax}[z] = \frac{\text{exp} [z_k]}{\sum_{k'=1}^K\text{exp}[z_{k'}]}  \tag{5.22}
$$
...
$$
Pr(y = k|x) = \mathrm{softmax}_{k}\left[f[x, \phi]\right]\tag{5.23}
$$
...
$$
 Pr(y|f[x, \phi])  = \prod_{d}Pr(y_{d}|f_{d}[x,\phi]\tag{5.25}
$$
...
$$
\begin{array}{rcl}\hat{\boldsymbol{\theta}}&=&\underset{\boldsymbol{\theta}}{\mathop{\text{argmin}}}\left[\int_{-\infty}^{\infty}q(y)\log\bigl[q(y)\bigr]dy-\int_{-\infty}^{\infty}q(y)\log\bigl[Pr(y|\boldsymbol{\theta})\bigr]dy\right]\\\\&=&\underset{\boldsymbol{\theta}}{\mathop{\text{argmin}}}\left[-\int_{-\infty}^{\infty}q(y)\log\bigl[Pr(y|\boldsymbol{\theta})\bigr]dy\right]\end{array}\tag{5.29}
$$
...
$$
\hat{\phi}=\underset{\phi}{\mathop{\text{argmin}}}\left[-\sum_{i=1}^I\log\left[Pr(y_i|\mathbf{f}[\mathbf{x}_i,\boldsymbol{\phi}])\right]\right] \tag{5.31}
$$
...
$$
L = -(1-y)\text{log}\left[1-\text{sig}\left[f[x,\phi]\right]\right] - y \text{log}\left[\text{sig}[f[x,\phi]]\right] \tag{5.33}
$$
...
$$
Pr(y\mid\mu,\kappa)=\frac{\exp[\kappa\cos(y-\mu)]}{2\pi\cdot\mathrm{Bessel}_{0}(\kappa)} \tag{5.34}
$$
