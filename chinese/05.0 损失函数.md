...
\begin{aligned}
\begin{aligned}\hat{ϕ}\quad\end{aligned}
&=\quad\underset{\boldsymbol{\phi}}{\mathop{\text{argmax}}}\left [\prod_{i=1}^IPr(\mathbf{y}_i|\mathbf{f}[\mathbf{x}_i,\mathbf{ϕ}]) \right] \\
&=\quad\underset{\boldsymbol{\phi}}{\mathop{\text{argmax}}}\left [\text{log}\left [\prod_{i=1}^IPr(\mathbf{y}_i|\mathbf{f}[\mathbf{x}_i,\mathbf{ϕ}]) \right]\right] \\
...
\begin{aligned}\hat{ϕ}\quad\end{aligned}
&=\quad\underset{\boldsymbol{\phi}}{\mathop{\text{argmax}}}\left [-\sum_{i=1}^I\text{log}\left [Pr(\mathbf{y}_i|\mathbf{f} [\mathbf{x}_i,\boldsymbol{\phi}]) \right]\right]\\
&=\quad\underset{\boldsymbol{\phi}}{\mathop{\text{argmax}}}\left [\text{L}[\mathbf{ϕ}] \right] \\
\end{aligned}\tag{5.4}
$$
...
&=\quad\underset{\boldsymbol{y}}{\mathop{\text{argmax}}}\left [Pr(\mathbf{y}|\mathbf{f}[\mathbf{x},\hat{\mathbf{ϕ}}]) \right]
\end{aligned}\tag{5.3}
$$
...
$$
\hat{ϕ} = \underset{\mathbf{ϕ}}{\text{argmin}}\left[ L[\mathbf{ϕ}] \right] = \underset{\mathbf{ϕ}}{\text{argmin}}\left[ -\sum_{i=1}^I\text{log}\left[ Pr(\mathbf{y_{i}}|f[\mathbf{x_i},\mathbf{ϕ}])\right] \right] \tag{5.6}
$$
...
$$
\hat{\mathbf{y}} = \underset{y}{\text{argmax}}\left[ Pr(y|f[x,\hat{ϕ}])
\right]\tag{5.12}
$$
...
因此，我们将分布参数预测为 $λ=\text{sig}[f[x, ϕ]]$。现在，似然函数为：

$$
Pr(y|x)  = (1 - \text{sig}[f [x, ϕ]])^{1-y} \cdot \text{sig}[f [x, ϕ]]^{y}\tag{5.19}
$$
...
出于将在[5.7节](#cross-entropy-loss)中解释的原因，这被称为*二元交叉熵损失*。变换后的模型输出 $\text{sig}[f[x, ϕ]]$ 预测了伯努利分布的参数 $λ$。这代表了 $y=1$ 的概率，因此 $1-λ$代表了 $y=0$ 的概率。当进行推断时，我们可能希望得到 $y$的一个点估计，因此如果 $λ>0.5$，则设置 $y=1$，否则设置 $y=0$。
...
$$
Pr(y = k|x)  = \text{softmax_{k}}\left[f[x, ϕ]\right]\tag{5.23}
$$
...
转换后的模型输出表示可能类别 $y ∈ \{1, 2, \ldots, K\}$的分类分布。对于一个点估计，我们取最可能的类别 $\hat{y} = \text{argmax}_{k}\left[Pr(y=k|f[x,\hat{ϕ}]) \right]$。这对应于图[5.10](#_bookmark184)中该值的曲线最高的位置。
...
$$
L = -(1-y)\text{log}\left[1-\text{sig}\left[f[x,ϕ]\right]\right] - y \text{log}\left[\text{sig}[f[x,ϕ]]\right] \tag{5.33}
$$

其中 $\text{sig}[•]$在方程[5.32](#_bookmark199)中定义。绘制该损失函数作为变换后的网络输出 $\text{sig}[f[x, ϕ]]∈ [0, 1]$的函数(i)当训练标签 $y = 0$时，(ii)当 $y = 1$时。
