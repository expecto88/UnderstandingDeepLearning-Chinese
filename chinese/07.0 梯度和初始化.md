...
考虑一个具有八个标量参数 $\phi = \{β_{0}, ω_{0}, β_{1}, ω_{1}, β_{2}, ω_{2}, β_{3}, ω_{3}\}$ 的模型 $f [x, ϕ]$ ，它由函数 $\text{sin}[•]$ ,  $\text{exp}[•]$ , 和  $\text{cos}[•]$ 的组合构成。
...
**问题 7.7** 考虑使用逻辑 sigmoid 函数（见方程 [7.37](#_bookmark297)）作为激活函数，即  $h =\text{sig}[f]$ 。计算该激活函数的导数  $∂h/∂f$ 。当输入取（i）一个较大的正值和（ii）一个较大的负值时，导数会发生什么变化？
