...
$$
\ell_{i} = -(1-y_{i})\log\left[1-\text{sig}\left[\text{f}[\mathbf{x}_{i},\phi]\right]\right] - y_{i}\log\left[\text{sig}\left[\text{f}[\mathbf{x}_{i},\phi]\right]\right] \tag{7.36}
$$
其中，函数  $\text{sig}[•]$  是逻辑 sigmoid 函数，定义为：
$$
\text{sig}[z] = \frac{1}{1+\text{exp}[z]} \tag{7.37}
$$
**问题 7.6**∗ 证明对于  $z = β + Ωh$ ，有：
$$
\frac{\partialz}{\partialh} = Ω^{T} \tag{7.42}
$$
