...
$$
\begin{array}{rcl}\\\\mathbf{h}_1&=&\mathbf{a}[\mathbf{\beta}_0+\mathbf{\Omega}_0\mathbf{x}]\\\\mathbf{h}_2&=&\mathbf{a}[\mathbf{\beta}_1+\mathbf{\Omega}_1\mathbf{h}_1]\\\\mathbf{h}_3&=&\mathbf{a}[\mathbf{\beta}_2+\mathbf{\Omega}_2\mathbf{h}_2]\\\\mathbf{f}[\mathbf{x},\boldsymbol{\phi}]&=&\mathbf{\beta}_3+\mathbf{\Omega}_3\mathbf{h}_3,\end{array}
\qquad\text{(7.1)}
$$
...
$$
L[\boldsymbol{\phi}]=\sum_{i=1}^I\ell_i \qquad\text{(7.2)}
$$

用于训练神经网络的最常用的优化算法是随机梯度下降（SGD），它更新参数如下：

$$
\phi_{t+1}\longleftarrow\phi_t-\alpha\sum_{i\in\mathcal{B}_t}\frac{\partial\ell_i[\phi_t]}{\partial\phi} \qquad\text{(7.3)}
$$

其中 $\alpha$ 是学习率， $\mathcal{B}_{t}$ 包含迭代 $t$ 时的批次索引。为了计算这个更新，我们需要计算导数：

$$
\frac{\partial\ell_i}{\partial\boldsymbol{\beta}_k}\quad\mathrm{and}\quad\frac{\partial\ell_i}{\partial\boldsymbol{\Omega}_k} \qquad\text{(7.4)}
$$
...
考虑一个具有八个标量参数 $\phi = \{β_{0}, ω_{0}, β_{1}, ω_{1}, β_{2}, ω_{2}, β_{3}, ω_{3}\}$ 的模型 $f [x, \phi]$ ，它由函数 $\text{sin}[•]$ ,  $\text{exp}[•]$ , 和  $\text{cos}[•]$ 的组合构成。
...
以及最小二乘损失函数 $L [\phi] = \sum_{i}ℓ_{i}$ ，其中每个项为。

$$
\ell_i=(\mathrm{f}[x_i,\phi]-y_i)^2 \qquad\text{(7.6)}
$$

其中， $x_{i}$ 是第 $i$ 个训练输入， $y_{i}$ 是第 $i$ 个训练输出。你可以将其看作是一个简单的神经网络，每层有一个输入、一个输出和一个隐藏单元，并且在每个层之间有不同的激活函数 $\text{sin}[•]$ ,  $\text{exp} [•]$ , 和  $\text{cos}[•]$ 。
...
> 图 7.4 反向传播的第一次反向传递: 我们从计算损失的函数末尾开始反向计算导数 $\partialℓ_i/\partialf_•$ 和 $\partialℓ_i/\partial_{h•}$，这些导数表示损失对中间量的变化。每个导数都是通过乘以形如 $\partialf_{k}/\partialh_{k}$ 或 $\partialh_{k}/\partialf_{k-1}$ 的项来计算的。
...
\quad&=\quad\frac{\partial h_1}{\partial f_0}\left(\frac{\partial f_1}{\partial h_1}\frac{\partial h_2}{\partial f_1}\frac{\partial f_2}{\partial h_2}\frac{\partial h_3}{\partial f_2}\frac{\partial f_3}{\partial h_3}\frac{\partial\ell_i}{\partial f_3}\right).\end{aligned}\qquad\text{(7.12)}
$$
...
$$
\begin{aligned}\frac{\partial\ell_i}{\partial\beta_k}&=&\frac{\partial f_k}{\partial\beta_k}\frac{\partial\ell_i}{\partial f_k}\\\\frac{\partial\ell_i}{\partial\omega_k}&=&\frac{\partial f_k}{\partial\omega_k}\frac{\partial\ell_i}{\partial f_k}\end{aligned}\qquad\text{(7.13)}
$$
...
> 图 7.5 反向传播的第二次反向传递。最后，我们计算导数 $\partialℓ_i/\partial β_{•}$ 和 $\partialℓ_i/\partialω_•$。每个导数都是通过将 $\partialℓ_{i}/\partialf_{k}$ 乘以相应的 $\partialf_{k}/\partialβ_{k}$ 或 $\partialf_{k}/\partialω_{k}$ 计算得出的。

这与前一节中的观察1一致；权重 $ω_{k}$ 的变化效应与源变量 $h_{k}$ 的值成比例（在前向传递中存储了） 。从术语 $f_{0} = β_{0} + ω·x_{i}$ 得到的最终导数为：

$$
\frac{\partial f_0}{\partial\beta_0}=1\quad\text{和}\quad\frac{\partial f_0}{\partial\omega_0}\quad=\quad x_i \qquad\text{(7.15)}
$$
...
- 损失函数  $ℓ_{i}$  对网络输出  $f_{3}$  的导数  $\partialℓ_{i} /\partial f_{3}$  会根据损失函数的不同而有不同的形式，但通常都很简单。
- 网络输出  $f_{3}$  对隐藏层  $h_{3}$  的导数  $\partialf_{3} /\partial h_{3}$  是。

$$
\frac{\partial\mathbf{f}_3}{\partial\mathbf{h}_3}=\frac\partial{\partial\mathbf{h}_3}\left(\boldsymbol{\beta}_3+\boldsymbol{\Omega}_3\mathbf{h}_3\right)=\boldsymbol{\Omega}_3^T\qquad\text{(7.20)}
$$

如果你对矩阵微积分不熟悉，这个过程可能不好理解。它在问题[7.6](#_bookmark298)中有详细探讨。

激活函数输出  $h_{3}$  对其输入  $f_{2}$  的导数  $\partialh_{3} /\partial f_{2}$  会根据激活函数的不同而有所不同。它将是一个对角矩阵，因为每个激活函数只依赖于相应的预激活。对于ReLU函数，对角线上的元素在  $f_{2}$  小于零的地方都是零，在其他地方都是一（图[7.6](#_bookmark273)）。我们不需要乘以这个矩阵，而是将对角线上的元素提取为向量  $\mathbb{I}[f_{2}> 0]$  并逐元素相乘，这样更高效。
...
**向后传递 #2：** 现在我们知道如何计算  $\partialℓ_{i} /\partial f_{k}$  ，我们可以专注于计算损失对权重和偏置的导数。为了计算损失对偏置  $β_{k}$  的导数，我们再次使用链式法则。
...
现在我们简要总结一下最终的反向传播算法。考虑一个深度神经网络  $f [x_{i}, \phi]$ ，它接受输入  $x_{i}$ ，有  $K$  个带有ReLU激活函数的隐藏层，并且具有损失项  $ℓ_{i} = l [f [x_{i}, \phi] , y_{i}]$ 。反向传播的目标是计算相对于偏置  $β_{k}$  和权重  $Ω_{k}$  的导数  $\partialℓ_{i} /\partial β_{k}$  和  $\partialℓ_{i} /\partial Ω_{k}$ 。

**前向传播：**我们计算并存储以下量：

$$
\begin{array}{rcl}\mathbf{f}_0&=&\mathbf{\beta}_0+\mathbf{\Omega}_0\mathbf{x}_i\\\\mathbf{h}_k&=&\mathbf{a}[\mathbf{f}_{k-1}]&k\in\{1,2,\ldots,K\}\\\\mathbf{f}_k&=&\mathbf{\beta}_k+\mathbf{\Omega}_k\mathbf{h}_k.&k\in\{1,2,\ldots,K\}\end{array}\qquad\text{(7.23)}
$$

**反向传播：** 我们从损失函数  $ℓ_{i}$  相对于网络输出  $f_{K}$  的导数  $\partialℓ_{i} /\partial f_{K}$  开始，然后向后逐层计算：
...
$$
 ...
 \quad&=\quad\mathbb{I}[\mathbf{f}_{k-1}>0]\odot\left(\Omega_k^T\frac{\partial\ell_i}{\partial\mathbf{f}_k}\right),\quad&k\in\{K,K-1,\ldots,1\}\quad\end{aligned}\qquad\text{(7.24)}
$$
...
$$
σ_{Ω}^{2} =\frac{2}{D_h}\qquad\text{(7.31)}
$$

其中 $D_{h}$ 是应用权重的原始层的维度。这被称为*He初始化*。
### 7.5.2 后向传播的初始化 - Initialization for backward pass
类似的论证可以得出在后向传播过程中梯度  $\partiall/\partialf_{k}$  的方差如何变化。在后向传播过程中，我们乘以权重矩阵的转置  $Ω^{T}$ （方程 [7.24](#_bookmark277) ），因此等价表达式变为：
...
\end{array} \qquad\text{(7.34)}
$$

其中，函数  $a[•]$  是 ReLU 函数。直接计算输出  $y$  对于每个参数  $\phi_{•}, \theta_{••}$  和  $ψ_{••}$  的导数（即不使用反向传播算法）。ReLU 函数对于其输入的导数  $\partiala[z]/\partialz$  是指示函数  $\mathbb{I}[z > 0]$ ，如果参数大于零则返回1，否则返回0（图 [7.6](#_bookmark273)）。
**问题 7.2** 在方程 [7.12](#_bookmark268) 的五个导数链中，找出每个链的最终项的表达式。
**问题 7.3** 方程 [7.19](#_bookmark275) 中的每个项的大小是多少？
**问题 7.4** 计算最小二乘损失函数的导数  $\partialℓ_i/\partialf[x_{i}, \phi]$ 。

$$
\ell_{i} = \left(y_{i} - \mathrm{f}[\mathbf{x}_{i},\phi]\right)^{2} \qquad\text{(7.35)}
$$

**问题 7.5** 计算二分类损失函数的导数  $\partialℓ_i/\partialf[x_{i}, \phi]$ 。

$$
\ell_{i} = -(1-y_{i})\log\left[1-\text{sig}\left[\text{f}[\mathbf{x}_{i},\phi]\right]\right] - y_{i}\log\left[\text{sig}\left[\text{f}[\mathbf{x}_{i},\phi]\right]\right] \qquad\text{(7.36)}
$$

其中，函数  $\text{sig}[•]$  是逻辑 sigmoid 函数，定义为：

$$
\text{sig}[z] = \frac{1}{1+\text{exp}[z]} \qquad\text{(7.37)}
$$

**问题 7.6**∗ 证明对于  $z = β + Ωh$ ，有：

$$
\frac{\partialz}{\partialh} = Ω^{T} \qquad\text{(7.42)}
$$

其中， $\partialz/\partialh$  是一个包含其  $i^{th}$  列和  $j^{th}$  行的项  $\partialz_i/\partialh_j$  的矩阵。为了证明这一点，首先找出元素  $\partialz_i/\partialh_j$  的表达式，然后考虑矩阵  $\partialz/\partialh$  必须采取的形式。
**问题 7.7** 考虑使用逻辑 sigmoid 函数（见方程 [7.37](#_bookmark297)）作为激活函数，即  $h =\text{sig}[f]$ 。计算该激活函数的导数  $\partialh/\partialf$ 。当输入取（i）一个较大的正值和（ii）一个较大的负值时，导数会发生什么变化？
**问题 7.8** 考虑使用（i）Heaviside 函数和（ii）矩形函数作为激活函数。

$$
\text{Heaviside}[z] = \begin{cases}0 & z < 0 \\\\ 1 & z \geq 0\end{cases} \qquad\text{(7.38)}
$$

和

$$
\text{rect}[z] = \begin{cases}0 & z < 0 \\\\ 1 & 0 \leq z \leq 1 \\\\ 0 & z > 1\end{cases} \qquad\text{(7.39)}
$$

讨论为什么这些函数对于使用基于梯度的优化方法进行神经网络训练是有问题的。
**问题 7.9**∗ 考虑一个损失函数  $ℓ[f]$ ，其中  $f=β+Ωh$ 。我们想要找出当我们改变  $Ω$  时损失  $ℓ$  如何变化，我们将用一个包含导数  $\partialℓ/\partialΩ_{ij}$  的矩阵来表示，该导数位于第  $i^{th}$  行和第  $j^{th}$  列。找出  $\partialf_i/\partialΩ_{ij}$  的表达式，并使用链式法则证明：

$$
\frac{\partial\ell}{\partial\boldsymbol{\Omega}} = \frac{\partial\ell}{\partial\mathbf{f}}\mathbf{h}^{T} \qquad\text{(7.40)}
$$

**问题 7.10**∗ 推导使用 leaky ReLU 激活函数的反向传播算法的反向传递方程，其定义为：

$$
\text{a}[z] = \text{ReLU}[z] = \begin{cases}\alpha\cdot z & z < 0 \\\\ z & z \geq 0\end{cases} \qquad\text{(7.41)}
$$
...
相关的计算图如图[7.9](#_bookmark302)所示。使用*反向模式微分*计算导数 $\partialy/\partialx$ 。换句话说，按顺序计算以下内容。
...
**问题7.13** 对于问题[7.42](#_bookmark307)中的相同函数，使用*正向模式微分*计算导数 $\partialy/\partialx$ 。换句话说，按顺序计算以下内容。
