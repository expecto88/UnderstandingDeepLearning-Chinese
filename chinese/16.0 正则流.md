...
标准化流是一种概率生成模型：它将一个概率分布拟合到训练数据中（图[14.2b]()）.考虑对一个1D分布 $Pr (x)$ 进行建模。标准化流从一个简单可计算的“基础”分布 $Pr (z)$ 开始，该分布定义了一个潜在变量 $z$ ，并应用一个函数 $x = \text{f} [ z, \phi]$ ，其中参数 $\phi$ 被选择为使得 $Pr ( x)$ 具有所需的分布（图[16.1]()）。生成一个新的示例 $x^{∗}$ 很容易；我们从基础密度中抽取 $z^{∗}$ ，并通过函数传递，以得到 $x^{∗} = \text{f} [z^{∗}, \phi]$ 。

![Alt text](../img/image-16.1.png)
> 图 16.1 转换概率分布。
>
> * a）基础密度是在一个潜在变量 $z$ 上定义的标准正态分布。
> * b) 这个变量经过函数 $x = \text{f} [ z, \phi]$ 的转换，得到了一个新的变量 $x$ ，
> * c) 其具有了一个新的分布。为了从这个模型中进行抽样，我们从基础密度中抽取了值 $z$ （面板（a）中的绿色和棕色箭头展示了两个示例）。然后，我们将这些值通过函数 $\text{f} [ z, \phi]$ （面板（b）中的虚线箭头展示）传递，生成了 $x$ 的值，这些值在面板（c）中用箭头表示。

## 16.1.1 测量概率 - Measuring probability

测量数据点 $x$ 的概率很有挑战性。考虑将函数 $\text{f} [ z, \phi]$ 应用于具有已知密度 $Pr (z)$ 的随机变量 $\mathbf z$ 上。函数会在被拉伸的区域导致概率密度减小，在被压缩的区域导致概率密度增加，以保持新分布下的总概率为一。函数 $\text{f} [ z, \phi]$ 拉伸或压缩其输入的程度取决于其梯度的大小。如果输入微小变化导致输出大幅变化，那么函数会拉伸。如果输入微小变化导致输出变化较小，那么函数会压缩（图[16.2]()）。
...
其中 $z = \text{f}^{-1} [x, \phi]$ 是生成 $x$ 的潜变量。潜变量在基础密度下的原始概率通过函数的导数大小进行调节。如果导数大于1，概率减小。如果导数小于1，概率增加。

### 16.1.2 正向映射和逆向映射 - Forward and inverse mappings

为了从分布中抽取样本，我们需要使用正向映射 $x = \text{f} [z, \phi]$ ，但为了计算似然性，我们需要计算逆向映射 $z = \text{f}^{-1} [ x, \phi]$ 。因此，我们需要选择合理的 $\text{f} [ z, \phi]$ ，使其能够进行逆向映射。
...
学习过程中，为了学习分布，我们寻找最大化训练数据 $\{x_{i}\}^I_{i=1}$ 的似然概率的参数 $\phi$ ，或者等价地最小化负对数似然：

$$
\begin{aligned}
\text{φ}& =\quad\underset{\phi}{\mathop{\text{argmax}}}\left[\prod_{i=1}^IPr(x_i|\boldsymbol{\phi})\right] \\\\
&=\quad\underset{\boldsymbol{\phi}}{\mathop{\text{argmin}}}\left[\sum_{i=1}^I-\log\left[Pr(x_i|\boldsymbol{\phi})\right]\right] \\\\
&=\quad\underset{\boldsymbol{\phi}}{\mathop{\text{argmin}}}\left[\sum_{i=1}^I\log\left[\left|\frac{\partial\mathrm{f}[z_i,\boldsymbol{\phi}]}{\partial z_i}\right|\right]-\log\Bigl[Pr(z_i)\Bigr]\right],
\end{aligned}
\qquad\text{(16.2)}
$$
...
其中 $\mathbf z = \mathbf f^{-1} [\mathbf x, \phi]$ 是潜在变量 $\mathbf z$ 生成 $\mathbf x$ 。第一项是雅可比矩阵 $\partial\text{f}[\mathbf z, \phi]/\partial\mathbf z$ 的行列式的倒数，该矩阵包含位置 $(i,j)$ 处的项 $\partial \text{f}_{i}[\mathbf z,\phi]/\partialz_{j}$ 。类似于绝对导数测量在应用函数时1D函数上某一点面积的变化，绝对行列式测量在多变量函数中某一点的体积变化。第二项是潜在变量在基础密度下的概率。

### 16.2.1 用神经网络来定义正向映射 - Forward mapping with a deep neural network

在实践中，我们通常使用神经网络来定义正向映射 $\text{f}[\mathbf z, \phi]$ ，它由一系列带有参数 $\phi_{k}$ 的层 $\text{f}_k[•, \phi_{k}]$ 组成，这些层按照顺序组合在一起：
...
逆向映射（归一化方向）由每个层的逆向操作 $\text{f}^{-1}_k [•, \phi_{k}]$ 按照相反的顺序应用而定义：
...
> 图16.4 深度神经网络的正向和逆向映射。基本密度（左侧）通过网络层 $\text{f}_{1}[• , \phi], \text{f}_{2}[•, \phi], \ldots$逐渐变换，形成模型密度。每一层都是可逆的，我们也可以等价地认为层的逆映射将模型密度逐渐“流动”回基本密度。
...
其中，我们重载了符号，使得 $\mathbf f_{k}$ 成为函数 $\mathbf f_K [•,\phi_{k}]$ 的输出。通过计算每个部分绝对行列式的乘积，可以得到该雅可比矩阵的绝对行列式：
...
$$
\begin{array}{rcl}\hat{\phi}&=&\underset{\phi}{\mathop{\text{argmax}}}\left[\prod_{i=1}^IPr(\mathbf{z}_i)\cdot\left|\frac{\partial\mathbf{f}[\mathbf{z}_i,\boldsymbol{\phi}]}{\partial\mathbf{z}_i}\right|^{-1}\right]\\\\&=&\underset{\phi}{\mathop{\text{argmin}}}\left[\underset{\phi}{\mathop{\sum}}^I\log\left[\left|\frac{\partial\mathbf{f}[\mathbf{z}_i,\boldsymbol{\phi}]}{\partial\mathbf{z}_i}\right|\right]-\log\left[Pr(\mathbf{z}_i)\right]\right],\end{array}
\qquad\text{(16.8)}
$$

其中 $\mathbf z_{i} = \mathbf f^{-1} [\mathbf x_{i}, \phi]$ ， $Pr(\mathbf z_{i})$ 是基础分布下的测量结果，绝对行列式 $|\partial\text{f} [\mathbf z_{i}, \phi]/\partial\mathbf z_{i}|$ 由方程[16.7]()给出。
...
线性流的形式为 $\text{f} [\mathbf h] = β + Ω\mathbf h$ 。如果矩阵 $Ω$ 是可逆的，则线性变换是可逆的。对于 $Ω ∈ \mathbb{R}^{D×D}$ ，反演的计算复杂度为 $\mathcal{O} [D^{3}]$ 。雅可比矩阵的行列式就是 $Ω$ 的行列式，也可以在 $\mathcal O [D^{3}]$ 中计算得到。这意味着随着维度 $D$ 的增加，线性流变得越来越昂贵。
...
很遗憾，线性流动的表达能力不足。当将线性函数 $\text{f} [\mathbf h] = β + Ω\mathbf h$ 应用于服从正态分布的输入 $\text{Norm}_{\mathbf h}[µ, Σ]]$ 时，结果也服从正态分布，其均值和方差分别为 $\mathbf β + \mathbf Ω \mathbf µ$ 和 $\mathbf Ω \mathbf Σ \mathbf Ω^{T}$ 。因此，仅仅使用线性流动无法将正态分布映射到任意密度。

### 16.3.2 逐元素流动 - Elementwise flows

由于线性流动的表达能力不足，我们必须转向非线性流动。其中最简单的是逐元素流动，它对输入的每个元素应用具有参数 $\phi$ 的逐点非线性函数 $\mathbf f [•, \phi]$ ，使得：
...
雅可比矩阵 $\frac{\partial f[\mathbf h]}{\partial \mathbf h}$ 是一个对角矩阵，因为第 $d^{th}$ 个输入对 $\text{f}[\mathbf h]$ 的第 $d^{th}$ 个输出有影响。它的行列式是对角线元素的乘积，所以：
...
$$
 ...
 $\mathbf f[•, \phi]$ 函数可以是一个固定的可逆非线性函数，比如泄漏的ReLU（图3.13），在这种情况下没有参数，或者它可以是任何参数化的可逆一对一映射。一个简单的例子是一个分段线性函数，有 $K$ 个区域（图16.5），它将[0, 1]映射到[0, 1]，如下所示：
...
这个函数中， $\phi_{1}, \phi_{2} , \ldots, \phi_{K}$ 是一系列正数，并且它们的和为 $K$ ，而 $b = ⌊Kh⌋$ 表示包含 $h$ 的块的索引。第一项表示前面所有块的总和，而第二项表示 $h$ 在当前块中的位置比例。这个函数很容易进行反转，并且在几乎任何地方都可以计算其梯度。还有许多类似的方案可以创建平滑的函数，通常使用参数化的样条曲线，以确保函数是单调的，从而可以进行反转。

![Alt text](../img/image-16.5.png)
> 图16.5 分段线性映射。将输入域 $h \in [0, 1]$ 等分为 $K$ 个大小相等的区域（这里 $K=5$ ），可以创建一个可逆的分段线性映射 $\mathbf h^′ = \mathbf f [h, \phi]$ 。每个区域具有参数 $\phi_k$ 控制斜率。
>
> * a) 如果这些参数是正数并且总和为一，则
...
*耦合流*将输入 $\mathbf{h}$ 分为两部分， $\mathbf{h} = [ \mathbf{h}_1^T,\mathbf{h}_2^T]^T$。对于流 $\text{f} [\mathbf{h}, \boldsymbol{\phi}]$ 的定义如下：
...
> 图 16.6 耦合流。
>
> * a）输入（橙色向量）被分为 $\mathbf h_{1}$ 和 $\mathbf h_{2}$ 两部分。输出（青色向量）的第一部分 $\mathbf h^′_1$ 是 $\mathbf h_{1}$ 的副本。通过将不可逆的变换 $\mathbf g [•, \phi]$ 应用于 $\mathbf h_{2}$ ，得到输出 $\mathbf h^′_2$ ，其中参数 $\phi$ 是 $\mathbf h_{1}$ 的一个（不一定可逆）函数。
> * b）在逆映射中， $\mathbf h_{1} = \mathbf h^′_1$ 。这使我们能够计算参数 $\phi [\mathbf h_{1}]$ ，然后应用逆变换 $\mathbf g^{-1} [\mathbf h^′_2, \phi]$ 来恢复 $\mathbf h_{2}$ 。

如果函数 $\mathbf g [•, \phi]$ 是逐元素变换，Jacobian 矩阵将为对角线矩阵，左上象限为单位矩阵，右下象限为逐元素变换的导数。其行列式为这些对角线值的乘积。
...
转换函数 $\mathbf g [•,•]$ 被称为*转换器* ^[1]^，而参数 $\phi, \phi [h_{1}], \phi [h_{1}, h_{2}], \ldots$ 被称为*条件器*。就耦合流而言，转换器 $\mathbf g [•, \phi]$ 必须是可逆的，而条件器 $\phi [•]$ 可以采用任何形式，通常是神经网络。如果转换器和条件器足够灵活，自回归流就是*通用的逼近器*，因为它们能够表示任何概率分布。

> ^[1]^ 这与第[12]()章讨论的转换器层无关。

可以通过使用适当的掩码网络以并行方式计算输出 $\mathbf h^′$ 的所有条目，使得位置 $d$ 上的参数 $\phi$ 仅依赖于先前的位置。这被称为*掩码自回归流*。这个原则与掩码自注意力（第[12.7.2]()节）非常相似；将输入与先前的输出相关联的连接被剪除。
...
其中 $\mathbf{f}_1[•, \phi_{1}]$ 和 $\mathbf{f}_2[•, \phi_{2}]$ 是两个不一定可逆的函数（见图[16.8]()）。通过颠倒计算顺序，可以计算出逆运算：
...
这个公式可以很容易地进行反转，但对于一般的函数 $\mathbf{f}_1[•, \phi_{1}]$ 和 $\mathbf{f}_2[•, \phi_{2}]$ 来说，没有有效的方法来计算雅可比矩阵。这种公式有时被用于在训练残差网络时节省内存；因为网络是可逆的，所以在正向传播中存储每层的激活是不必要的。

### 16.3.7 残差流与收缩映射：iResNet - Residual flows and contraction mappings: iResNet

利用残差网络的另一种方法是应用Banach不动点定理或收缩映射定理，该定理指出每个收缩映射都有一个不动点。收缩映射 $\text{f} [•]$ 具有以下特性：

$$
\text{dist}\Big[\text{f}[z'],\text{f}[z]\Big]<\beta\cdot\text{dist}\Big[z',z\Big]\quad\quad\forall z,z',
\qquad\text{(16.20)}
$$

其中 $\text{dist}[•, •]$ 是一个距离函数， $0<β<1$ 。当这样的函数被反复应用时（即，将输出反复作为输入），结果会收敛到一个不动点，即 $\text{f} [z] = z$ （如图[16.9]()所示）。理解这一点，可以考虑将函数应用于不动点和当前位置；不动点保持不变，但两者之间的距离必须变小，因此当前位置必须接近不动点。
![Alt text](../img/image-16.9.png)
> 图 16.9 收缩映射。如果一个函数在每个点的斜率都小于一，不断迭代该函数会收敛到一个固定点 $\text{f} [z] = z$ 。
>
> * a）从初始点 $z_{0}$ 开始，我们计算出 $z_{1} = \mathbf f [z_{0}]$ 。然后将 $z_{1}$ 再次输入函数进行迭代。最终，这个过程会收敛到满足 $\text{f} [z] = z$ 的点（即函数与虚线对角线相交的位置）。
> * b）这可以用于求解形如 $y = z + f [z]$ 的方程，通过观察到 $y^{∗} - \text{f} [z]$ 的不动点（橙色线与虚线相交的位置）与 $y^{∗} = z + f [z]$ 的解位置相同。
...
前提是 $\text{f} [z]$ 是一个收缩映射。换句话说，我们可以用这个定理找到对应给定值 $y^{∗}$ 的 $z^{∗}$ 。方法是从任意点 $z_{0}$ 开始迭代 $z_{k+ 1} = y^{∗}-\text{f} [z_{k}]$ 。这个迭代过程会在 $z + \text{f}[z] = y^{∗}$ 处收敛（参见图 [16.9b]()）。

如果我们确保 $\text{f} [\mathbf h, \phi]$ 是一个收缩映射，那么同样的原理可以用于反转形式为 $\mathbf h^′ = \mathbf h + \mathbf f [\mathbf h, \phi]$ 的残差网络层。实际上，这意味着Lipschitz常数必须小于1。假设激活函数的斜率不大于1，这相当于确保每个权重矩阵 $\mathbf Ω$ 的最大特征值小于1。一种简单的方法是通过裁剪权重 $\mathbf Ω$ 的绝对值来保持其小。
...
在第一行中，我们使用了等式 $\log[|\mathbf A|] = \text{trace}[\text{log}[A]]$ ，并在第二行将其展开为幂级数形式。

即使我们截断了这个级数，计算每个项的迹（ $\text{trace}$）仍然非常耗时。因此，我们使用*Hutchinson的迹估计器*来近似计算。假设有一个均值为0，方差为 $\mathbf I$的正态随机变量 $\mathbf ϵ$。我们可以通过以下方式估计矩阵 $\mathbf A$的迹：
...
&=\quad\mathbb{E}\left[\text{trace}\left[\mathbf{A\epsilon\epsilon}^T\right]\right] \\\\
...
> 图 16.10 多尺度流。
> 在正常化流中，潜空间 $\mathbf z$ 的大小必须与模型密度相同。然而，它可以被分成几个组件，并逐渐引入到不同的层中。这样做可以加快密度估计和采样的速度。对于逆过程，黑色箭头反转，每个块的最后一部分都跳过剩余的处理。例如， $\mathbf f^{-1}_3 [•, \phi_3]$ 只对前三个块进行操作，第四个块成为 $\mathbf z_{4}$ 并与基础密度进行比较评估。
...
为了取得进展，我们从学生模型中生成样本 $\mathbf x_{i} = \mathbf f [\mathbf z_{i}, \phi]$ 。由于这些样本是我们自己生成的，我们知道它们对应的潜在变量 $\mathbf z_{i}$ ，并且可以在不进行反演的情况下计算它们在学生模型中的似然。因此，我们可以使用一种像是掩码自回归流的模型，其中反演速度较慢。我们定义了一个基于反向KL散度的损失函数，鼓励学生模型和教师模型的似然相同，并使用它来训练学生模型（图[16.14)]()）：

$$
\hat{\boldsymbol{\phi}}=\underset{\boldsymbol{\phi}}{\mathop{\text{argmin}}}\left[\mathrm{KL}\left[\sum_{i=1}^I\delta\big[\mathbf{x}-\mathrm{f}[\mathbf{z}_i,\boldsymbol{\phi}]\big]\bigg|\bigg|q(\mathbf{x})\bigg]\right].\right.
\qquad\text{(16.25)}
$$
...
> * c) 或者，我们可以修改流参数 $\phi$，通过最小化从流样本 $x_{i} = \text{f} [z_i, \phi]$ 到
> * d) 目标密度的KL散度来拟合最大似然。

这种方法与通常使用归一化流来构建一个概率模型 $Pr (x_{i} , \phi)$ 来拟合未知分布的数据样本 $\mathbf x_{i}$ 的最大似然方法形成对比。最大似然方法依赖于正向KL散度中的交叉熵项（第[5.7节](#cross-entropy-loss)）：

$$
\hat{\phi}=\underset{\boldsymbol{\phi}}{\mathop{\text{argmin}}}\left[\underset{i}{\mathop{\mathrm{KL}}}\left[\frac1I\sum_{i=1}^I\delta[\mathbf{x}-\mathbf{x}_i]\Big|\Big|Pr(\mathbf{x}_i,\boldsymbol{\phi})\right]\right].
\qquad\text{(16.26)}
$$
...
**问题 16.1** 考虑使用函数 $x = \text{f} [z] = z_{2}$ 将定义在 $z [0, 1]$ 上的均匀基础密度进行转换。求转换后的分布 $Pr ( x)$ 的表达式。
...
**问题16.3** ∗ 根据方程 $\mathbf z = \text{f}^{-1} [\mathbf x, \phi]$ ，求出反演映射雅可比矩阵的表达式，并计算该雅可比矩阵的绝对行列式，使其形式与方程 [16.6]() 和 [16.7]() 相似。
...
**问题16.6** 证明如果 $\mathbf x = \mathbf f [\mathbf z] = \mathbf A\mathbf z + \mathbf b$ ，且 $Pr (\mathbf z) = \text{Norm}_{z}[\mathbf µ, \mathbf Σ]$ ，则 $Pr (\mathbf x) = \text{Norm}_{x}[\mathbf A\mathbf µ+\mathbf b, \mathbf A\mathbf Σ\mathbf A^{T} ]$ ，这是通过使用如下关系式得到的:
...
请写出 leaky ReLU 的反函数表达式，以及对于多元变量 $\mathbf z$ 的逐元素变换 $\mathbf x = \mathbf f [z]$ 的雅可比行列式的反绝对值 $|\partial\mathbf f[\mathbf z]/\partial\mathbf z|^{-1}$ 的表达式，其中:

$$
\mathbf{f}[\mathbf{z}]=\begin{bmatrix}\text{LReLU}[z_1],\text{LReLU}[z_2],\ldots,\text{LReLU}[z_D]\end{bmatrix}^T.
$$

**问题16.8** 考虑将方程[16.12]()中定义的分段线性函数 $\text{f} [h, \phi]$ 应用于定义域为 $h^′\in [0, 1]$ 的输入向量 $\mathbf h = [h_{1}, h_{2}, \ldots, h_D]^{T}$ ，使得输出向量 $\text{f} [\mathbf h] = [\text{f} [h_{1}, \phi], \text{f} [h_{2}, \phi] , \ldots, \text{f} [h_D, \phi]]$ 。请问 Jacobian 矩阵 $\partial\text{f} [\mathbf h]/\partial\mathbf h$ 是什么？Jacobian 矩阵的行列式是多少？
...
其中 $b = ⌊Kh⌋$ 是 $h$ 所在的箱体，参数 $\phi_{k}$ 是正数，并且总和为一。考虑当 $K = 5$ 且 $\phi_{1} = 0.1, \phi_{2} = 0.2 , \phi_{3} = 0.5 , \phi_{4} = 0.1 , \phi_{5} = 0.1$ 的情况。绘制函数 $\text{f} [h, \phi]$ 。绘制反函数 $\text{f}^{-1} [h^′, \phi]$ 。
